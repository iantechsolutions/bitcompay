...
.payment {
  ...
  height: 40px;
    ...
    <div class="conceptotables-container" style="height: 280px;">
       ...
       ${totalTributes > 0 ?  (`<div class="tribute-grid" style="display: grid; grid-template-columns: repeat(5, auto);grid-auto-rows: min-content;">
    <div><p>TRIBUTO</p></div>
    ...
    ${comprobante.otherTributes
      .map(
        (tribute: any) =>
          ...
          <div><span>${formatNumberAsCurrency(tribute.base_imponible)}</span></div>
...
  </div>`): ("<div style='width: 70px;'></div>")}
  ...
  <div style="display:flex; flex-direction:column; margin-bottom: 10px; font-weight:400;">
		<p style="font-size:12px; text-align:right; margin-right:0; padding-right:0;">Sub-total: ${formatNumberAsCurrency(subtotal ?? 0)}</p>
    <p style="font-size:12px; text-align:right;  margin-right:0; padding-right:0; padding-top:5px;">IVA: ${formatNumberAsCurrency(iva ?? 0)}</p>
		${totalTributes > 0 ? (`<p style="font-size:12px; text-align:right;  margin-right:0; padding-right:0; padding-top:5px;">Otros Tributos: ${formatNumberAsCurrency(totalTributes ?? 0)}</p>`) : ("")}
...
function numeroALetras(numero: number | undefined): string {
  ...
  const convertirParteEntera = (numero: number): string => {
    ...
    if (numero < 1000) {
      ...
      return (
        centenas[centena] + (resto !== 0 ? " " + convertirParteEntera(resto) : "")
      );
    }
    if (numero < 1000000) {
  ...
  if (numero) {
    ...
    return capitalizarPrimeraLetra(resultadoParteEntera) + ` con ${parteDecimal}/100`;
  }
