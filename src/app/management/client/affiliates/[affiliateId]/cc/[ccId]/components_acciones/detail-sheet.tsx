import { ChevronDown, ChevronRight, FileText } from "lucide-react";
import Link from "next/link";
import { Button } from "~/components/ui/button";
import {
  Table,
  TableCell,
  TableHead,
  TableHeader,
  TableRow,
} from "~/components/ui/tablePreliq";
import {
  Sheet,
  SheetContent,
  SheetDescription,
  SheetHeader,
  SheetTitle,
  SheetTrigger,
} from "~/components/ui/sheet";
import { RouterOutputs } from "~/trpc/shared";
import { useState } from "react";
import { api } from "~/trpc/react";
import ContentTable from "~/app/billing/liquidation/[liquidationId]/content-table";
import { TableRecord } from "../columns";
import { formatNumberAsCurrency } from "~/lib/utils";
import { mostrarNroComprobante } from "../columns";
type DetailSheetProps = {
  data?: TableRecord;
  open: boolean;
  setOpen: (open: boolean) => void;
};

export default function DetailSheet({ data, open, setOpen }: DetailSheetProps) {
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetContent className="sm:max-w-[550px] px-10 py-12 overflow-y-scroll">
        <SheetHeader>
          <SheetTitle className="font-semibold text-2xl">
            Detalle del Comprobante
          </SheetTitle>
          <SheetDescription>
            <ul className="mt-2">
              <li className="text-xs"> RECEPTOR </li>
              <li className="font-medium-medium text-[#3e3e3e]">
                {data?.nombre ?? "-"}
              </li>
              <br />
              <li className="text-xs"> CUIL/CUIT </li>
              <li className="font-medium-medium text-[#3e3e3e]">
                {data?.cuit ?? "-"}
              </li>
              <br />
            </ul>
          </SheetDescription>
        </SheetHeader>
        {data && data["Tipo comprobante"] == "Apertura de CC" ? (
          <div className="flex flex-row border justify-between items-center px-4 py-5 gap-2 rounded-md mt-3">
            <p className="text-base whitespace-nowrap font-medium-medium "></p>
            <p
              className={`text-[#6952EB] whitespace-nowrap font-semibold text-lg${
                Number(data?.amount) > 0 ? "text-[#6952EB]" : "text-[#EB2727]"
              }`}
            >
              {" "}
              {data?.amount}
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            <div className="flex flex-row border justify-between items-center px-8 py-5 gap-2 rounded-xl mt-3">
              <div className="font-semibold text-lg"> Comprobante </div>
              <div className="flex flex-col h-full justify-center items-start gap-1 pr-6">
                <div className="opacity-80 text-xs">{data?.["Tipo comprobante"] ?? ""} №</div>
                <div className="text-[#6952EB] font-bold text-xl">
                  {" "}
                  {mostrarNroComprobante(
                    data?.ptoVenta ?? 0,
                    data?.comprobanteNumber ?? 0
                  )}
                </div>
              </div>
            </div>
            {data?.comprobantes && (
              <ContentTable comprobante={data?.comprobantes} />
            )}
            <div className="bg-[#DEF5DD] flex flex-row justify-between items-center py-4 px-6 rounded-md mt-4">
              <p className=" text-[#6952EB] font-semibold">Importe total: </p>
              <p className="text-[#6952EB] font-semibold">
                {formatNumberAsCurrency(data?.saldo_a_pagar ?? 0)}
              </p>
            </div>
          </div>
        )}
      </SheetContent>
    </Sheet>
  );
}
