...
export const columns: ColumnDef<TableRecord>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const processDate = row.getValue("process_date") as Date | null;
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const contributionDate = row.getValue("contribution_date") as Date | null;
  ...
  {
    accessorKey: "support_date",
    header: "Fecha de Soporte",
    cell: ({ row }) => {
      const supportDate = row.getValue("support_date") as Date | null;
      return supportDate ? dayjs(supportDate).format("DD/MM/YYYY") : "N/A";
    },
  },
  {
    id: "actions",
