...
export default async function Page(props: { params: { productId: string } }) {
  const product = await api.products.get.query({
    productId: props.params.productId,
  });
  const channels = await api.channels.list.query();
  const companies = await api.companies.list.query();
  if (!product) {
    return <Title>No se encontró el producto</Title>;
  }

  return (
    <ProductPage product={product} channels={channels} companies={companies} />
  );
}
