        ...
        <>
                        ...
                        {props.status_batch
                            ...
                            .map((row) => (
                                <TableRow key={row.product as React.Key}>
                                    <TableCell className='font-medium'>{typeof row.status === 'string' ? row.status : ''}</TableCell>
                                    <TableCell>{typeof row.records === 'number' ? row.records : ''}</TableCell>
                                    <TableCell>
                                        {typeof row.amount_collected === 'number' ? '$'.concat(row.amount_collected.toString()) : ''}
                                    </TableCell>
                                </TableRow>
                            ))}
                    </TableBody>
            ...
            <Dialog>
                <DialogTrigger asChild={true}>
                    <Button disabled={isLoading || props.status_batch[0]!.records === 0 || disabled} size='lg' className='w-full'>
                ...
                <DialogContent className='sm:max-w-[425px]'>
                    ...
                    <div className='grid gap-4 py-4'>
                        <div className='grid grid-cols-4 items-center gap-4'>
                            ...
                            <Input id='fileName' value={fileName} onChange={handleName} className='col-span-3' />
