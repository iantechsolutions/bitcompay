...
type UpdateProductsBatchFunction = (
...
export function ReceiveDataProvider(props: { children: React.ReactNode }) {
  const [productsBatchArray, setProductsBatchArray] = useState<
  ...
  return (
    <ReceiveDataContext.Provider value={contextValue}>
      {props.children}
    </ReceiveDataContext.Provider>
  );
}
...
export function useReceiveData() {
  ...
  if (context === null) {
    throw new Error(
      "useReceiveData must be used within Received data provider",
    );
  }
  return context;
}
