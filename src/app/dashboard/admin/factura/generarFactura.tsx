...
export async function ingresarAfip() {
    ...
    const _alias = 'afipsdk2'
    const _afipCuit = new Afip({
        CUIT: taxId,
        access_token: 'T11zSjRqweUhefsFkp0rn1jlvY2KyX1zRo4aRVpmfLR5fowH0kov709vL6Zn9i1F',
        production: true,
    })
...
export function FacturaDialog({ receivedHtml }: FacturaDialog) {
    ...
    async function generateFactura() {
        ;(async () => {
            ...
            const _ivas = await afip.ElectronicBilling.getAliquotTypes()

            const _serverStatus = await afip.ElectronicBilling.getServerStatus()

            let last_voucher
            try {
                last_voucher = await afip.ElectronicBilling.getLastVoucher(puntoVenta, tipoFactura)
            } catch {
                last_voucher = 0
            }
            ...
            const data = {
                ...
                CbteFch: Number.parseInt(fecha?.replace(/-/g, '') ?? ''),
            ...
            const _resHtml = await afip.ElectronicBilling.createPDF({
                html: html,
                file_name: name,
                options: options,
            })
