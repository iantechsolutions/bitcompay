...
export default async function Brand(props: { params: { brandId: string } }) {
    ...
    const unrelatedCompanies= await api.companies.getUnrelated.query({brandId:props.params.brandId})

    if (!brand || !session?.user) {
        return <Title>No se encontró la marca</Title>
    }

    return <BrandPage brand={brand} companies={companiesRelated} unrelatedCompanies={unrelatedCompanies} />
}