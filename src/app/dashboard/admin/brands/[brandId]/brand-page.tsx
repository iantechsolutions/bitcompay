...
export default function BrandPage({
    ...
    const [reducedDescription, setReducedDescription] = useState(brand.redescription)
    const [relCompanies, setRelCompanies] = useState(new Set(relatedCompanies))

    const { mutateAsync: changeBrand, isLoading } = api.brands.change.useMutation()
    ...
    async function handleChange() {
        try {
            ...
            const nonNullRelatedCompanies = companiesIdArray.filter((company) => company !== undefined)
                        ...
                        <AccordionContent>
                            <List>
                                {companies?.map((company) => {
                                    const isChecked = Array.from(relCompanies).some((c) => c?.id === company?.id)
                                    return (
                                        <ListTile
                                            key={company?.id}
                                            title={company?.name}
                                            trailing={
                                                <Switch
                                                    checked={isChecked}
                                                    onCheckedChange={(required) => changeCompany(company, required)}
                                                />
                                            }
                                        />
                                    )
...
function Deletebrand(props: { brandId: string }) {
    const { mutateAsync: deletebrand, isLoading } = api.brands.delete.useMutation()
    ...
    return (
        <AlertDialog>
            ...
            <AlertDialogContent>
                <AlertDialogHeader>
                    <AlertDialogTitle>¿Estás seguro que querés eliminar la marca?</AlertDialogTitle>
                    <AlertDialogDescription>Eliminar marca permanentemente.</AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
