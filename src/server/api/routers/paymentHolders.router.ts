...
export const paymentHoldersRouter = createTRPCRouter({
    ...
    change: protectedProcedure
        ...
        .mutation(async ({ input: { id, ...input } }) => {
            const updatedpaymentHolders = await db.update(schema.paymentHolders).set(input).where(eq(schema.paymentHolders.id, id))
    ...
    delete: protectedProcedure
        ...
        .mutation(async ({ input }) => {
            await db.delete(schema.paymentHolders).where(eq(schema.paymentHolders.id, input.paymentHoldersId))
        }),
})
