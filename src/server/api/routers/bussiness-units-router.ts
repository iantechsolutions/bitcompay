...
export const bussinessUnitsRouter = createTRPCRouter({
    ...
    delete: protectedProcedure.input(z.object({ bussinessUnitId: z.string() })).mutation(async ({ input }) => {
        const bussinessUnit_deleted = await db.delete(schema.bussinessUnits).where(eq(schema.bussinessUnits.id, input.bussinessUnitId))
