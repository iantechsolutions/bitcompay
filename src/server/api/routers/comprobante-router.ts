...
const preparedAddPayment = db
  ...
  .values({
    ...
    second_due_amount: sql.placeholder("second_due_amount"),
    second_due_date: sql.placeholder("second_due_date").getSQL(),
        ...
        if (comprobante?.origin == "Nota de credito") {
          ...
          if (comprobanteAnterior?.estado !== "Error") {
            ...
            data = {
              ...
              ImpTotConc: 0,
              ImpNeto: (Number(comprobante?.importe) / (1 + ivaFloat)).toFixed(
                2
              ),
              ImpOpEx: 0,
              ImpIVA: (
          ...
          if (comprobanteEstado != "Error") {
            ...
            await db.insert(schema.payments).values({
              ...
              second_due_date: comprobante?.second_due_date,
              second_due_amount: comprobante?.importe,
  ...
  createPreLiquidation: protectedProcedure
    .input(
      z.object({
        ...
        second_due_date: z.date().optional(),
        ...
        if (family_group) {
          ...
          const payment = await db
            ...
            .values({
              ...
              second_due_date: updatedProvider[0]?.second_due_date,
              second_due_amount: updatedProvider[0]?.importe,
              cbu: billResponsible?.pa[0]?.CBU,
...
export async function preparateComprobante(
  ...
  second_due_date: Date | undefined,
  pv: string,
