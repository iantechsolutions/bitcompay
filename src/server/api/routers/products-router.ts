...
export const productsChannel = createTRPCRouter({
        ...
        await db.transaction(async tx => {
            ...
            if (input.channels) {
                ...
                const channelsToAdd = input.channels.filter(channelId => {
                    return !productChannels.find(productChannel => productChannel.channelId === channelId)
                })


                if (channelsToDelete.length > 0) {
                    await tx.delete(schema.productChannels).where(and(
                        eq(schema.productChannels.productId, input.productId),
                        inArray(schema.productChannels.channelId, channelsToDelete.map(productChannel => productChannel.channelId))
                    ))
                }

                if (channelsToAdd.length > 0) {
                    await tx.insert(schema.productChannels).values(channelsToAdd.map(channelId => ({
    ...
    delete: protectedProcedure.input(z.object({
        ...
        await db.transaction(async tx => {
            await tx.delete(schema.productChannels).where(eq(schema.productChannels.productId, input.productId))
            await tx.delete(schema.products).where(eq(schema.products.id, input.productId))
        })
    }),
})