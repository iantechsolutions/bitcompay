...
function generateDebitoDirecto(
    ...
    for (const transaction of transactions) {
        ...
        const fiscalNumber = formatString(' ', transaction.fiscal_id_number!.toString(), 22, true)
        if (!transaction.cbu) {
            throw new TRPCError({
                code: 'BAD_REQUEST',
        ...
        const invoice_number = formatString('0', transaction.invoice_number!.toString(), 15, false)
        const CBU = transaction.cbu
        text += `421002513  ${fiscalNumber}${CBU}${collectedAmount}00    ${period}${dateYYYYMMDD}  ${invoice_number}${' '.repeat(127)}\r\n`
        let name
        ...
        const concept = formatString(' ', transaction.additional_info ?? '', 40, true)
        text += `424002513  ${fiscalNumber}${concept}${' '.repeat(177)}\r\n`

        total_records += 4
        total_operations += 1
        total_collected += transaction.collected_amount ?? transaction.first_due_amount ?? 0
    }

    total_records += 1

    const total_collected_string = formatString('0', total_collected?.toString(), 15, false)
    const total_operations_string = formatString('0', total_operations?.toString(), 8, false)
    const total_records_string = formatString('0', total_records?.toString(), 10, false)
...
function generatePagomiscuentas(
    ...
    for (const transaction of transactions) {
        // registro
        const fiscal_id_number = formatString(' ', transaction.fiscal_id_number!.toString(), 19, true)
        const invoice_number = formatString(' ', transaction.invoice_number.toString(), 20, true)
        const first_due_date = dayjs(transaction.first_due_date).format('YYYYMMDD')
        const first_due_amount = formatString('0', transaction.first_due_amount ? transaction.first_due_amount?.toString() : '', 9, false)
    ...
    // trailer
    const total_collected_string = formatString('0', total_collected.toString(), 9, false)
    const total_records_string = formatString('0', total_records.toString(), 7, false)
    text += `9400${companyCode}${dateAAAAMMDD}${total_records_string}${'0'.repeat(7)}${total_collected_string}00${'0'.repeat(
        11,
    )}${'0'.repeat(48)}\n`
...
async function generatePagoFacil(
    ...
    let total_collected = 0
    for (const transaction of transactions) {
        const fiscal_id_number = formatString(' ', transaction.fiscal_id_number!.toString(), 11, true)
        const invoice_number = formatString('0', transaction.invoice_number.toString(), 6, false)
        const first_due_date = dayjs(transaction.first_due_date).format('DDMMYYYY')
        const first_due_amount = formatString('0', transaction.first_due_amount ? transaction.first_due_amount?.toString() : '', 10, false)
        ...
        const first_due_amount_bar_code = formatString('0', transaction.first_due_amount!.toString(), 6, false)
        ...
        text += `3${date}${'0'.repeat(6)}${invoice_number}10${formatString(
            ...
            60,
            true,
        )}${fiscal_id_number}PES${first_due_amount}00${terminal_id}${date}${hours}${seq_terminal} PESE${' '.repeat(29)}${formatString(
            '0',
            transaction.first_due_amount ? transaction.first_due_amount?.toString() : '',
            13,
            false,
        )}00\r\n`
        total_records++
    ...
    const total_records_string = formatString('0', total_records.toString(), 7, false)
    const total_collected_string = formatString('0', total_collected.toString(), 12, false)
    text += `9${'0'.repeat(8)}${'0'.repeat(12)}${'0'.repeat(7)}${total_records_string}${total_collected_string}${'0'.repeat(143)}`
    return text
}
function generateRapiPago(
    ...
    for (const transaction of transactions) {
        ...
        const first_due_date = dayjs(transaction.first_due_date).format('YYYYMMDD')
        const first_due_amount = formatString('0', transaction.first_due_amount ? transaction.first_due_amount?.toString() : '', 9, false)
        const second_due_date = dayjs(transaction.second_due_date ? transaction.second_due_date : transaction.first_due_date).format(
            'YYYYMMDD',
        )
        const second_due_amount = formatString(
            '0',
            transaction.second_due_amount ? transaction.second_due_amount?.toString() : '',
            9,
            false,
        )
    ...
    const total_records_string = formatString('0', total_records.toString(), 7, false)
    const total_collected_string = formatString('0', total_collected.toString(), 9, false)
    text += `981400${currentDate}${total_records_string}${'0'.repeat(7)}${total_collected_string}00${'0'.repeat(11)}${'0'.repeat(40)}`
    return text
}

function formatString(char: string, string: string, limit: number, final: boolean) {
    if (final) {
