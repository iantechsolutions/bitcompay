...
export const uploadsRouter = createTRPCRouter({
    ...
    readUploadContents: protectedProcedure
        ...
        .mutation(async ({ input }) => {
            return await db.transaction(async (db) => {
                ...
                const contents = await readUploadContents(db, input.id, input.type, input.companyId, channels, brands)
    ...
    readResponseUploadContents: protectedProcedure
        ...
        .mutation(async ({ input }) => {
            return await db.transaction(async (db) => {
                const contents = await readResponseUploadContents(db, input.uploadId, input.type)
    ...
    confirmResponseUpload: protectedProcedure
        ...
        .mutation(async ({ input }) => {
            await db.transaction(async (tx) => {
                const { records } = await readResponseUploadContents(tx, input.uploadId, undefined)
    ...
    confirmUpload: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
            await db.transaction(async (tx) => {
                const channels = await getCompanyProducts(input.companyId)
                const brands = await getCompanyBrands(input.companyId)
                const result = await readUploadContents(tx, input.id, 'rec', input.companyId, channels, brands)
                if (result.rows) {
    ...
    delete: protectedProcedure
        ...
        .mutation(async ({ input }) => {
            await db.delete(schema.documentUploads).where(eq(schema.documentUploads.id, input.uploadId))
        }),
})
        ...
        if (recordIndex === 1) {
            ...
            if (invoice_number) {
                const original_transaction = await db.query.payments.findFirst({
                    where: eq(schema.payments.invoice_number, Number.parseInt(invoice_number)),
                })
...
async function readUploadContents(
    ...
    let auxiliarInvoiceNumber = invoice_number_array.length > 0 ? Math.max(...invoice_number_array) : 0
    for (let i = 0; i < transformedRows.length; i++) {
        ...
        if (row.g_c) {
            ...
            if (!isInBrands) {
                errors.push(`Esta Marca: ${row.g_c} es invalido o  no se encuentra habilitado (fila:${rowNum})`)
            }
        ...
        if (product) {
            ...
            for (const column of product.requiredColumns) {
                ...
                if (!value) {
                    ...
                    //   reason: "Empty cell",
                    // });
                    errors.push(`La columna ${columnName} es obligatoria y no esta en el archivo(fila:${rowNum})`)
                }
            }
        }
    ...
    const TableHeaders = recHeaders.filter(
        (header) => companyReqColumns.has(header.key) || header.key === 'g_c' || header.key === 'product_number',
    )
